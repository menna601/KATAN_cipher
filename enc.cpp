...
// x_64 & y_64 are specific bits only used for 64-bit enc
...
void katan32_encrypt(const uint32_t plain[32], uint32_t cipher[32], const uint32_t key[80]) {
	...
	* fa & fb are functions to calcualte bit 0 of L2, L1 respectively
...
void katan64_encrypt(const uint64_t plain[64], uint64_t cipher[64], const uint64_t key[80]) {
	/*
	* L2: is a 39-bit register initialized with plain from bit 0  ---> bit 38
	* L1: is a 25-bit register initialized with plain from bit 39 ---> bit 63
	* fa0 & fa1 & fa2 are functions to calculate the value to be stored in bit 0, 1, 2 of L2 respectively
	& fb0 & fb1 & fb2 are functions to calculate the value to be stored in bit 0, 1, 2 of L1 respectively
	* k is array containing 254 bits = no. of rounds needed
	* i & j  are counters
	*/
	...
	for (i = 0; i < 254; ++i) {
		...
		//Calculate fa0, fa1, fa2, fb0, fb1, fb2 bits
		...
		//shift the 2 registers by 3 bits for each
		for (j = 24; j > 2; --j)
			L1[j] = L1[j - 3];
		for (j = 38; j > 2; --j)
			L2[j] = L2[j - 3];

		// Update the first 3 bits of L1 with fb0, fb1, fb2 
		L1[2] = fb_2;
		L1[1] = fb_1;
		L1[0] = fb_0;

		// Update the first 3 bits of L2 with fa0, fa1, fa2 
		L2[2] = fa_2;
		L2[1] = fa_1;
		L2[0] = fa_0;
	}
